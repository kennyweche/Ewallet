...
def main():
    ...
    url = ""
    queue_ = "EWALLET_PUSH_NOTIFY_QUEUE"
    log_path = "/var/log/flask/roamtech/PushNotifier.log"
    #server = '172.19.1.3'
    server = 'localhost'
    port = 5672
    ...
    pr = PushNotifier(username, password, url, queue_, server, port, rabbit_mq_user, rabbit_mq_pass, log_path)
    try:
        pr.run()
    except:
